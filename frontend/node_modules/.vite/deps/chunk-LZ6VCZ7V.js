import {
  encode
} from "./chunk-BDWYZ6PT.js";
import {
  getBytecode
} from "./chunk-Z5TXSCOX.js";
import {
  formatBlock,
  hexToBytes,
  readContract
...
async function download(options) {
  ...
  if (options.uri.startsWith("ar://")) {
    const { resolveArweaveScheme } = await import("./arweave-J7D3H7SG.js");
...
async function resolvePluginPatternAddresses(contract) {
  try {
    const { readContract: readContract2 } = await import("./read-contract-J6NKYQXQ.js");
...
async function estimateGas(options) {
  ...
  const promise = (async () => {
    ...
    const { encode: encode2 } = await import("./encode-SBD5OVHS.js");
    ...
    const [{ getRpcClient: getRpcClient2 }, { eth_estimateGas }] = await Promise.all([
      import("./rpc-ZHFC57VI.js"),
      import("./eth_estimateGas-44QBP4AD.js")
    ]);
...
async function toSerializableTransaction(options) {
  ...
  let [data, nonce, gas, feeData, to, accessList, value] = await Promise.all([
    ...
    (async () => {
      ...
      return from ? await import("./eth_getTransactionCount-FD2Y3VYK.js").then(({ eth_getTransactionCount }) => eth_getTransactionCount(rpcRequest, {
        address: from,
        blockTag: "pending"
      })) : void 0;
    })(),
...
//# sourceMappingURL=chunk-LZ6VCZ7V.js.map
